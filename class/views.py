    ...
    if request.method == "POST":
        ...
        try:
            with transaction.atomic():
                ...
                if result['success']:
                    messages.success(request, f"[OK] Successfully created grouped session for {len(classes_list)} classes with 150 shared sessions")
                    return redirect("class_sections_list")
                else:
                    messages.error(request, f"[ERROR] Error creating grouped session: {result.get('error', 'Unknown error')}")
                    return render(request, "admin/classes/bulk_add.html", {
                        'selected_classes': selected_classes,
                        'selected_ids': selected_ids,
                    })
...
def initialize_class_sessions(request, class_section_id):
    ...
    BOOST: Now supports shared curriculum via GroupedSession to save space and prevent duplicates.
    """
    if request.user.role.name.upper() != "ADMIN":
        messages.error(request, "Permission denied.")
        return redirect("no_permission")
    ...
    if request.method == "POST":
        try:
            with transaction.atomic():
                # IMPROVED CHECK: Check for ANY sessions (active or inactive) to prevent 300+ session duplication
                existing_sessions = PlannedSession.objects.filter(class_section=class_section)
                if existing_sessions.exists():
                    messages.warning(
                        request, 
                        f"Class already has {existing_sessions.count()} sessions. No duplicates created."
                    )
                    return redirect("class_sections_list_by_school", class_section.school.id)
                
                # SHARED CURRICULUM BOOST:
                # Check if another class in the same school and grade level already has a GroupedSession
                from django.apps import apps
                GroupedSession = apps.get_model('class', 'GroupedSession')
                
                existing_group = GroupedSession.objects.filter(
                    class_sections__school=class_section.school,
                    class_sections__class_level=class_section.class_level
                ).distinct().first()
                
                if existing_group:
                    # Just add this class to the existing group
                    existing_group.class_sections.add(class_section)
                    
                    # Also link the existing sessions to this class if they aren't already
                    master_class = existing_group.class_sections.exclude(id=class_section.id).first()
                    if master_class:
                        master_sessions = PlannedSession.objects.filter(class_section=master_class)
                        for s in master_sessions:
                            # Note: In a true shared model, PlannedSession might not have class_section
                            # But here we clone them for the new class to keep the per-class view,
                            # but we mark them with the same grouped_session_id for the 'Boost'.
                            s.pk = None
                            s.class_section = class_section
                            s.save()
                    
                    messages.success(request, f"Linked to shared curriculum for Grade {class_section.class_level}")
                else:
                    # Create all 150 sessions from scratch (Standard)
                    sessions_to_create = []
                    ...
                    for day_number in range(1, 151):
                        session = PlannedSession(
                            ...
                            is_required=True,
                            is_active=True,
                            grouped_session_id=new_group_id
                        )
                        sessions_to_create.append(session)
                    
                    PlannedSession.objects.bulk_create(sessions_to_create)
                    messages.success(request, f"Successfully initialized 150 sessions for {class_section}")
                
                logger.info(f"Admin initialized sessions for {class_section}")
    ...
    if request.method == "POST":
        try:
            with transaction.atomic():
                ...
                message = f"[OK] Successfully deleted {total_planned} planned session(s)"
        ...
        try:
            with transaction.atomic():
                ...
                if initialized_count > 0:
                    messages.success(
                        request, 
                        f"[OK] Successfully initialized sessions for {initialized_count} class(es) in {school.name}. "
    ...
    if request.method == "POST":
        try:
            with transaction.atomic():
                ...
                # Provide detailed feedback
                message = f"[OK] Successfully deleted {total_planned} planned session(s) from {len(classes_affected)} class(es) in {school.name}"
                if total_actual_sessions > 0:
                    message += f" and {total_actual_sessions} actual session(s) with {total_attendance} attendance record(s)"
...
def initialize_class_sessions(request):
    ...
    if request.method == "POST":
        try:
            ...
            if success_count > 0:
                messages.success(request, f"[SUCCESS] Successfully initialized {total_created} sessions for {success_count} classes.")
        ...
        try:
            ...
            if existing_sessions_count > 0:
                ...
                if result['success']:
                    messages.success(
                        request, 
                        f"[SUCCESS] Successfully initialized {result['created_count']} sessions for {class_section.class_level}-{class_section.section}"
                    )
    ...
    if request.method == "POST":
        try:
            with transaction.atomic():
                ...
                messages.success(
                    request, 
                    f"[DELETE] Successfully deleted all sessions for {class_section.class_level}-{class_section.section}. "
    ...
    if request.method == "POST":
        try:
            ...
            if total_created > 0:
                messages.success(
                    request, 
                    f"[START] Bulk initialization completed for {school.name}! "
    ...
    if request.method == "POST":
        try:
            with transaction.atomic():
                ...
                messages.success(
                    request, 
                    f"[DELETE] Successfully deleted ALL sessions for {school.name}! "
